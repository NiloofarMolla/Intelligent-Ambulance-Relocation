...
INCIDENT_LOG = SCRIPT_DIR / "incident_log_cross_700.csv"
COVERAGE_LOG = SCRIPT_DIR / "coverage_log_cross_700.csv"
SUMMARY_LOG = SCRIPT_DIR / "response_summary_cross_700.csv"
OUT_OF_SERVICE_LOG = SCRIPT_DIR / "out_of_service_log_cross_700.csv"
REWARD_LOG = SCRIPT_DIR / "rl_reward_log_cross_700.csv"
...
RATES_PER_HOUR = {
    (0, 6): 10, (6, 9): 45, (9, 12): 25, (12, 16): 25, (16, 20): 45, (20, 24): 40,
}
...
def generate_reports(df_resp, df_cov, df_unc, df_rew):
    print("\n--- Generating RL Evaluation Reports (High Demand - 30-Day Avg) ---")
    if not df_resp.empty:
        ...
        }).to_csv(SCRIPT_DIR / "KPI_summary_cross_700.csv", index=False)
    ...
    if not df_unc.empty:
        ...
        stats.round(2).to_csv(SCRIPT_DIR / "Node_Coverage_Stats_cross_700.csv", index=False)
    ...
    if not df_resp.empty:
        ...
        plt.title('Response Times (RL on 700 Demand - 30 Runs)', fontsize=14)
        plt.xlabel('Response Time (minutes)', fontsize=12);
        plt.ylabel('Count (Over 30 Days)', fontsize=12);
        plt.legend()
        plt.savefig(SCRIPT_DIR / "chart_response_times_cross_700.png", dpi=150, bbox_inches='tight');
        plt.close()
    ...
    if not df_cov.empty:
        ...
        plt.title('Average Uncovered Nodes (RL on 700 Demand - 30 Runs)', fontsize=14)
        plt.xlabel('Simulation Minute (0 = 06:00)', fontsize=12);
        plt.ylabel('Mean Uncovered Nodes', fontsize=12)
        plt.grid(True, ls=':', alpha=0.6)
        plt.savefig(SCRIPT_DIR / "chart_uncovered_nodes_cross_700.png", dpi=150, bbox_inches='tight');
        plt.close()
    ...
    if not df_rew.empty:
        ...
        plt.title('Average RL Cumulative Reward (700 Demand - 30 Runs)', fontsize=14)
        plt.grid(True, ls=':', alpha=0.6);
        ...
        plt.savefig(SCRIPT_DIR / "chart_rl_rewards_eval_cross_700.png", dpi=150, bbox_inches='tight');
        plt.close()
...
def main():
    ...
    print(f"Running 30 iterations for RL Agent (CROSS-EVAL 700 DEMAND)...")
    ...
    print("\n✅ Simulation and Statistical Reports for RL Agent (700) completed successfully!")
