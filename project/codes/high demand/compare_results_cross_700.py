...
FILE_BASELINE = SCRIPT_DIR / "response_summary_2_700.csv"
FILE_RL = SCRIPT_DIR / "response_summary_cross_700.csv"
...
def main():
    if not FILE_BASELINE.exists() or not FILE_RL.exists():
        print("Error: Files not found. Run simulations first.")
        return
    ...
    ax.set_title('Impact of RL in High Demand 700 (30-Run Statistical Avg)', fontsize=14, fontweight='bold')
    ax.set_xticks(x);
    ...
    save_path = SCRIPT_DIR / "chart_comparison_baseline_vs_cross_700_STATISTICAL.png"
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Chart saved as: {save_path.name}")
    plt.close()

    table_data = {
        "Traffic Condition (700 Demand)": conditions,
    ...
    df_table.to_csv(SCRIPT_DIR / "statistical_comparison_table_cross_700.csv", index=False)
    print("\n--- Formal Academic Table (High Demand 700) ---")
