...
# Config for Scenario 2 (High Demand 700)
OUT_EDGES_PATH = SCRIPT_DIR / "edges_with_traffic.csv"
INCIDENT_LOG = SCRIPT_DIR / "incident_log_2_700.csv"
COVERAGE_LOG = SCRIPT_DIR / "coverage_log_2_700.csv"
SUMMARY_LOG = SCRIPT_DIR / "response_summary_2_700.csv"
OUT_OF_SERVICE_LOG = SCRIPT_DIR / "out_of_service_log_2_700.csv"
...
# 🌟 نرخ حوادث برای رسیدن به مرز بحران
RATES_PER_HOUR = {
...
def generate_reports(df_resp, df_cov, df_unc):
    print("\n--- Generating Scenario 2 Reports (High Demand - 30-Day Avg) ---")
    if not df_resp.empty:
        ...
        }).to_csv(SCRIPT_DIR / "KPI_summary_2_700.csv", index=False)
    ...
    if not df_unc.empty:
        ...
        stats.round(2).to_csv(SCRIPT_DIR / "Node_Coverage_Stats_2_700.csv", index=False)
    ...
    if not df_resp.empty:
        ...
        plt.title('Distribution of Response Times (Baseline 700 Demand - 30 Runs)', fontsize=14)
        plt.xlabel('Response Time (minutes)', fontsize=12);
        plt.ylabel('Count (Over 30 Days)', fontsize=12);
        plt.legend()
        plt.savefig(SCRIPT_DIR / "chart_response_times_2_700.png", dpi=150, bbox_inches='tight');
        plt.close()
    ...
    if not df_cov.empty:
        ...
        plt.title('Average Uncovered Nodes over 24 Hours (Baseline 700 - 30 Runs)', fontsize=14)
        plt.xlabel('Simulation Minute (0 = 06:00)', fontsize=12);
        plt.ylabel('Mean Uncovered Nodes', fontsize=12)
        plt.grid(True, linestyle=':', alpha=0.6)
        plt.savefig(SCRIPT_DIR / "chart_uncovered_nodes_2_700.png", dpi=150, bbox_inches='tight');
        plt.close()
...
def main():
    ...
    print(f"Running 30 iterations for Baseline Scenario (HIGH DEMAND 700)...")
    ...
    print("\n✅ Simulation and Statistical Reports for Baseline (700) completed successfully!")
