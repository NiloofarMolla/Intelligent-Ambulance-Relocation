...
COVERAGE_FILE = SCRIPT_DIR / "Node_Coverage_Stats_rl.csv"
RESPONSE_FILE = SCRIPT_DIR / "response_summary_rl.csv"
NODES_FILE = SCRIPT_DIR / "nodes.csv"


def main():
    print("\n--- Generating Advanced Defense Analytics (30-Day Averages) ---")
    node_mapping = {}
    ...
    if RESPONSE_FILE.exists():
        ...
        # 1. گره‌های آسیب پذیر (قبلاً در test_rl میانگین‌گیری شده)
        if COVERAGE_FILE.exists():
            ...
            plt.title('Top 10 Most Vulnerable Nodes (Average Uncovered Min/Day)', fontsize=14, fontweight='bold')
            plt.xlabel('Node Name / Location', fontsize=12);
            plt.ylabel('Avg Uncovered Time per Day (Minutes)', fontsize=12)
            plt.xticks(rotation=45, ha='right');
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.savefig(SCRIPT_DIR / "chart_defense_vulnerable_nodes.png", dpi=300, bbox_inches='tight');
            plt.close()
        ...
        if not df_failed.empty:
            ...
            plt.title('Failed Missions by Hour of Day (Average per Day)', fontsize=14, fontweight='bold')
            plt.xlabel('Hour of Day', fontsize=12);
            plt.ylabel('Avg Number of Failed Missions', fontsize=12)
            plt.xticks(range(0, 24));
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.savefig(SCRIPT_DIR / "chart_defense_fails_by_hour.png", dpi=300, bbox_inches='tight');
            plt.close()
            ...
            plt.title('Top 10 Nodes with Highest Mission Failures (Avg per Day)', fontsize=14, fontweight='bold')
            plt.xlabel('Node Name / Location', fontsize=12);
            plt.ylabel('Avg Number of Failed Missions', fontsize=12)
            plt.xticks(rotation=45, ha='right');
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.savefig(SCRIPT_DIR / "chart_defense_fails_by_node.png", dpi=300, bbox_inches='tight');
            plt.close()
