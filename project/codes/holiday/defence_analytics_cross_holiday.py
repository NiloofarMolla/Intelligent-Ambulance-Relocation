...
COVERAGE_FILE = SCRIPT_DIR / "Node_Coverage_Stats_cross_holiday.csv"
RESPONSE_FILE = SCRIPT_DIR / "response_summary_cross_holiday.csv"
NODES_FILE = SCRIPT_DIR / "nodes.csv"


def main():
    print("\n--- Generating Advanced Defense Analytics (HOLIDAY - 30-Day Avg) ---")
    node_mapping = {}
    ...
    if RESPONSE_FILE.exists():
        ...
        # 1. گره‌های آسیب پذیر (قبلا در test_rl میانگین گیری شده)
        if COVERAGE_FILE.exists():
            ...
            plt.title('Top 10 Most Vulnerable Nodes (Avg Uncovered Min/Day - Holiday)', fontsize=14, fontweight='bold')
            plt.xlabel('Node Name / Location', fontsize=12);
            plt.ylabel('Avg Uncovered Time per Day (Minutes)', fontsize=12)
            plt.xticks(rotation=45, ha='right');
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.savefig(SCRIPT_DIR / "chart_defense_vulnerable_nodes_cross_holiday.png", dpi=300, bbox_inches='tight');
            plt.close()

        # 2. تحلیل شکست‌ها (تقسیم بر 30 روز)
        ...
        if not df_failed.empty:
            ...
            plt.title('Failed Missions by Hour of Day (Avg per Day - Holiday)', fontsize=14, fontweight='bold')
            plt.xlabel('Hour of Day', fontsize=12);
            plt.ylabel('Avg Number of Failed Missions', fontsize=12)
            plt.xticks(range(0, 24));
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.savefig(SCRIPT_DIR / "chart_defense_fails_by_hour_cross_holiday.png", dpi=300, bbox_inches='tight');
            plt.close()
            ...
            plt.title('Top 10 Nodes with Highest Mission Failures (Avg per Day - Holiday)', fontsize=14,
                      fontweight='bold')
            plt.xlabel('Node Name / Location', fontsize=12);
            plt.ylabel('Avg Number of Failed Missions', fontsize=12)
            plt.xticks(rotation=45, ha='right');
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            plt.savefig(SCRIPT_DIR / "chart_defense_fails_by_node_cross_holiday.png", dpi=300, bbox_inches='tight');
            plt.close()
