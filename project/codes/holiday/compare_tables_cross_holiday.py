...
def main():
    print("\n" + "=" * 60)
    print("GENERATING 30-DAY AVERAGED COMPARATIVE BENCHMARK TABLES (HOLIDAY)")
    print("=" * 60)

    h_metrics = []
    m1 = get_metrics("Baseline (No AI) - Holiday", SCRIPT_DIR / "coverage_log_2_holiday.csv",
                     SCRIPT_DIR / "Node_Coverage_Stats_2_holiday.csv", SCRIPT_DIR / "response_summary_2_holiday.csv")
    if m1: h_metrics.append(m1)

    m2 = get_metrics("RL Agent - Holiday", SCRIPT_DIR / "coverage_log_cross_holiday.csv",
                     SCRIPT_DIR / "Node_Coverage_Stats_cross_holiday.csv",
                     SCRIPT_DIR / "response_summary_cross_holiday.csv")
    if m2: h_metrics.append(m2)

    if h_metrics:
        df_h = pd.DataFrame(h_metrics)
        df_h.to_csv(SCRIPT_DIR / "Comparison_Table_cross_Holiday_30Avg.csv", index=False)
        print("\n--- Holiday Comparison Table (Average of 30 Runs) ---")
